...
public class ReplicateStatusUpdate {
    ...
    public ReplicateStatusUpdate(ReplicateStatus status,
        ...
        if (modifier != null && modifier.equals(ReplicateStatusModifier.POOL_MANAGER)) {
           this.date = new Date();
        }
    }
    ...
    public static ReplicateStatusUpdate poolManagerRequest(ReplicateStatus status) {
        return new ReplicateStatusUpdate(status, ReplicateStatusModifier.POOL_MANAGER);
    }

    public static ReplicateStatusUpdate poolManagerRequest(ReplicateStatus status,
                                                           ReplicateStatusCause cause) {
        ReplicateStatusDetails details = new ReplicateStatusDetails(cause);
        return new ReplicateStatusUpdate(status, ReplicateStatusModifier.WORKER, details);
    }

    public static ReplicateStatusUpdate poolManagerRequest(ReplicateStatus status,
                                                           ReplicateStatusDetails details) {
        return new ReplicateStatusUpdate(status, ReplicateStatusModifier.POOL_MANAGER, details);
    }
}